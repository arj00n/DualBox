...
5. Confirm:
   ...
   - `artifacts\publish\win-x64\DualBox.exe` exists.

## Phase 2: Controller bridge

1. Run `dotnet run --project .\src\DualBox.App\DualBox.App.csproj -c Release`.
...
2. Add the running `DualBox.exe` path to the Applications list.
...
3. Confirm DualBox opens automatically.
4. If it does not, check `%APPDATA%\DualBox\settings.json` and the `HKCU\Software\Microsoft\Windows\CurrentVersion\Run` registry key.

## Phase 4: Forza validation

1. Keep DualBox running.
